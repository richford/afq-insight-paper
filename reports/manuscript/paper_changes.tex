...
right CST, plotted over the FA values for the control and ALS subject groups \added{(see supplemental Figures S1, S2 for tract profiles of all 18 tracts in these groups)}.
...
imaging (DKI) \cite{jensen2005diffusion} for the HBN and Cam-CAN datasets \added{see supplemental Figures S3-S8 for the full tract profile information in all tracts/datasets}.
...
\added{slower, more performant models afterwards. The consistent \texttt{Scikit Learn}-based application programming interface of \texttt{AFQ-Insight} provides a good basis for this pattern.}
...
with a twice refocused spin-echo sequence and the same preprocessing \added{and reconstruction} pipeline
