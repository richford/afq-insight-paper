...
performance.
\added{ We approach differences between individuals through a ``decoding'' lens:}
We predict the phenotypical variance in a group of subjects (or classify group membership), based on a linear combination of the features estimated with \replaced{tract profiling}{tractometry}
\added{This is unlike many previous models of white matter differences (e.g., TBSS \cite{smith2006tract} and connectometry \cite{yeh2016connectometry}) that are ``encoding'' models, where differences between individuals and groups are tested as hypotheses about differences in their brain features (i.e., in their tract profiles; the distinction between ``encoding'' and ``decoding'' and the terms stem from functional MRI literature \cite{Naselaris2011-ss})}.
...
\added{One more approach to multivariate analysis of neuroimaging data is to perform inference or prediction in a transformed feature space rather then the original diffusion metric feature space.}
\added{For example, in Lasso PCR, diffusion features can be projected onto a principal components (PC) basis for use in a sparsity constrained model \mbox{\cite{powell2018local, rasero2021integrating}}.}
\added{While these approaches achieve high predictive performance and may even yield biologically interpretable principal components \mbox{\cite{chamberland2019dimensionality}}, they neglect anatomical grouping information.}
\added{We view these approaches as complementary to the SGL-based approach: PCR based approaches seeks to model brain-phenotype relationships using}
\added{the most parsimonious representation of variance in the diffusion measures. In contrast, our SGL-based approach seeks to establish whether prior knowledge of anatomical grouping improves modeling of brain-phenotype relationships.}
\added{Motivated by this distinction, we also introduce a union of the two approaches called PCR-SGL, in which each group of features is independently transformed into its PC basis, thereby retaining the anatomical grouping information of}
\added{the original feature space. We demonstrate circumstances in which this approach both helps and hinders predictive performance.}
...
engineering.
\added{We also predicted ALS diagnosis using the PCR-SGL, wherein each}
\added{group of features is independently transformed to its PC basis and}
\added{an SGL model is fit to the transformed features. This model achieves}
\added{\protect{\alsAccuracyGpca}\% accuracy and an ROC AUC of \protect{\alsRocAucGpca}}
(\cref{fig:class-results:probs}). In addition to this
classification performance, both SGL \added{and PCR-SGL} also \replaced{identify}{identifies}
...
\added{The relatively poor performance of Lasso PCR suggests that features with small variance are nonetheless relevant for predicting ALS diagnosis.}
\added{Or equivalently, that group differences between ALS diagnoses are not the dominant source of variance in the diffusion metrics.}
...
\added{Another limitation of our approach is in its computational execution time,}
\added{which, in our experience, is roughly five times slower than simpler models}
\added{like the lasso. This slow-down is likely to be insignificant for ``production''}
\added{cycles in which researchers report their findings, but may be prohibitive}
\added{for ``development'' cycles in which researchers repeatedly train models while}
\added{adjusting their analysis pipeline. In practice, we circumvent this limitation}
\added{by developing our analysis infrastructure with fast, simple models, and substituting}
\added{slower, more performant models afterward.}
...
\added{See Figure S3 of \mbox{\cite{kruper2021evaluating}} for}
\added{a depiction of these white matter bundles.}
Here,
...
\added{The resulting feature space was the same for all four datasets, with}
\added{the FA and MD metrics at each of 100 nodes in eighteen bundles}
\added{comprising 3600 features per subject.}
...
\added{SGL may be combined with principal components regression (PCR-SGL) by}
\added{performing dimensionality reduction separately for each group of covariates.}
\added{Let}
\begin{equation}
    \mathbf{X}^{(\ell)} = \mathbf{U} \mathbf{\Sigma} \mathbf{V}^T,
    \label{eq:svd}
\end{equation}
\added{be the compact singular value decomposition (SVD) of \protect$\mathbf{X}^{(\ell)}$,}
\added{the \protect$n \times p_\ell$ submatrix of \protect$\mathbf{X}$ corresponding to group}
\added{\protect$\ell$. Here, \protect$\mathbf{\Sigma}$ is an \protect$r \times r$ matrix, where}
\added{\protect$r = \min(n, p_\ell)$, that contains the non-zero singular values of}
\added{\protect$\mathbf{X}^{(\ell)}$. \protect$\mathbf{V}^T$ is an \protect$r \times p$ semi-orthogonal}
\added{matrix containing the principal axes of \protect$\mathbf{X}^{(\ell)}$. The product}
\added{\protect$\mathbf{Z} = \mathbf{U \Sigma}$ is an \protect$n \times r$ matrix containing the}
\added{principal component row vectors needed to reproduce \protect$\mathbf{X}^{(\ell)}$}
\added{in the basis provided by \protect$\mathbf{V}^T$.}

\added{Since this decomposition is performed separately for each group of covariates,}
\added{the grouping information is preserved in the transformation from \protect$\mathbf{X}$}
\added{to \protect$\mathbf{Z}$. We may then build an SGL model relating \protect$y$}
\added{and \protect$\mathbf{Z}$,}
...
\begin{align}
    ...
    \label{eq:pcr-sgl}
\end{align}
\added{The PCR-SGL coefficients \protect$\hat{\theta}$ may be projected back onto the}
\added{original feature space using \protect$\hat{\beta} = \mathbf{V} \hat{\theta}$.}
...
\added{This is similar to the use of a link function in a generalized}
\added{linear model, but without the adoption of an exponential probability}
\added{distribution \mbox{\cite{nelder1972technique}}.}
...
\end{equation}
\added{implemented using scikit-learn's \lstinline|TransformedTargetRegressor|}
\added{meta-estimator \mbox{\cite{scikit-learn}}}
\added{with numpy's \lstinline|log| and \lstinline|exp| as the transform and}
\added{inverse transform functions, respectively \mbox{\cite{harris2020array}}.}
