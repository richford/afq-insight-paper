...
\begin{figure}[b!]
    ...
    \textbf{(a)} Whole brain tractography generates streamlines approximating
        ...
        The coefficients in $\hat{\beta}$ follow the same natural grouping. \added{Panels (a) and (b) are adapted from \url{https://figshare.com/articles/figure/example_tractography-segmentation/14485350}, and reproduced under the CC-BY license (\url{https://creativecommons.org/licenses/by/4.0/})}
    }
\end{figure}
...
right CST, plotted over the FA values for the control and ALS subject groups \added{(see Supporting Figures S1 and S2 for tract profiles of all 18 tracts in these groups)}.
...
\begin{figure}[b!]
    ...
    \caption{
    {\bf Figure 2: \added{PCR-}SGL accurately and interpretably predicts ALS diagnosis.}
...
imaging (DKI) \cite{jensen2005diffusion} for the HBN and Cam-CAN datasets \added{see Supporting Figures S3 to S8 for the full tract profile information in all tracts/datasets}.
...
\replaced{tract profile}{tractometry} in the {\bf (d)} WH, {\bf (e)} HBN, and
...
\begin{figure}[b!]
    ...
    \caption{%
    {\bf \added{Figure 4: Model performance across all datasets.}}
...
\added{We present age quintile bundle profiles and SGL \protect$\beta$ coefficients for all four datasets in the Supporting Information. For age regression, substantial differences between the datasets preclude generalization across all three:}
\begin{enumerate*}[%
    label=(\roman*),%
    before=\unskip{: },%
    itemjoin={{, }},%
...
area\added{: Rutgers University Brain Imaging Center and the CitiGroup Cornell Brain Imaging Center. Age distributions of participants in the different sites are in Supporting Figure S9}.
...
tracts\added{, which are enumerated in Figure S1,} were identified
