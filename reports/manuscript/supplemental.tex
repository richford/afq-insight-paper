...
\todo{Ariel: This section seems incomplete. I am pretty sure there was some motion correction, eddy current correction and so forth happening in here.
Also, we might want to drop the part that describes calculation of confounds,
since we aren't using them.}
...
by \cite{power_fd_dvars}. The head-motion estimates calculated in the
...
\begin{multline}
    ...
    \label{eq:sgl}
\end{multline}
$G$ is the number of groups $\mathbf{X}^{(\ell)}$ is the submatrix of
...
comparing $\hat{\beta}$ across models. Ariel: agreed!}
