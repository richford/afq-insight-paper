...
right CST, plotted over the FA values for the control and ALS subject groups (see supplemental Figures S1, S2 for tract profiles of all 18 tracts in these groups).
...
imaging (DKI) \cite{jensen2005diffusion} for the HBN and Cam-CAN datasets see supplemental Figures S3-S8 for the full tract profile information in all tracts/datasets.
...
See Figure S3 of \mbox{\cite{kruper2021evaluating} for a depiction of these white matter bundles.}
Here,
...
This is similar to the use of a link function in a generalized linear model, but without the adoption of an exponential probability distribution \mbox{\cite{nelder1972technique}.}
...
\end{equation}
implemented using scikit-learn's \lstinline|TransformedTargetRegressor| meta-estimator \mbox{\cite{scikit-learn} with numpy's \lstinline|log| and \lstinline|exp| as the transform and inverse transform functions, respectively \mbox{\cite{harris2020array}}.}
